...
class model(object):
    def __init__(
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope('this-scope') as scope:
                self.right_lstm_cell = rnn.BasicLSTMCell(num_units=self.hidden_Units, activation=self.activation)
                self.left_lstm_cell = rnn.BasicLSTMCell(num_units=self.hidden_Units, activation=self.activation)
                ...
                out = tf.multiply(normalize_out1, normalize_out2)

                with tf.name_scope("last-layer"):
                    self.W_f = tf.get_variable(
                        "W_f",
                        shape=[2 * self.max_sequence_length * self.hidden_Units, self.total_classes],
                ...
                with tf.name_scope("loss"):
                    self.loss = tf.reduce_mean(tf.square(self.final_score - self.labels))
    ...
    def get_char_emb(self, chars_x):
        ...
        char2word_embeddings = tf.reshape(tf.squeeze(tf.concat(pooled_outputs, axis=3)),
                                          [self.batch_size, self.max_sequence_length, self.total_filters])
