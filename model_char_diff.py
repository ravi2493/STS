    ...
    def __init__(
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope('this-scope') as scope:
                ...
                # l.append(out1)
                # l.append(out2)
                dot_ = tf.multiply(out1,out2)
                diff_ = tf.abs(tf.subtract(out1,out2))
                l.append(dot_)
                l.append(diff_)
                out_appnd = tf.reshape(tf.stack(l,axis=1),[-1,2*2*self.hidden_Units * self.max_sequence_length])
                with tf.name_scope("last-layer"):
                    self.W_f = tf.get_variable(
                        "W_f",
                        shape=[2*2*self.hidden_Units * self.max_sequence_length, self.total_classes],
                    ...
                    self.final_score = tf.nn.xw_plus_b(out_appnd, weights=self.W_f, biases=bias, name="scores")
